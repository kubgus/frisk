...
size_tree iterate(const std::filesystem::path& path);

size_tree process_entry(const std::filesystem::directory_entry& entry) {
    try {
...
size_tree iterate(const std::filesystem::path& path) {
    ...
    for (const auto& entry : std::filesystem::directory_iterator(path, std::filesystem::directory_options::skip_permission_denied)) {
        futures.push_back(std::async(std::launch::async, process_entry, entry));
    }
